...
def plot_scores(filename="sac_scores.npy", window=50):
    ...
    plt.plot(scores, label="Episode Reward", alpha=0.3, color='purple')
    ...
    if len(scores) >= window:
        ...
        plt.plot(np.arange(len(rolling_avg)) + window - 1, rolling_avg,
                 label=f"{window}-Episode Moving Average", color='purple', linewidth=2)

    plt.ylabel("Score")
    plt.xlabel("Episode")
    plt.title("SAC Training Progress (LunarLanderContinuous-v3)")
