...
def plot_scores(filename="dueling_dqn_lunar_scores.npy", window=50):
    ...
    if len(scores) >= window:
        ...
        plt.plot(np.arange(len(rolling_avg)) + window - 1, rolling_avg,
                 label=f"{window}-Episode Moving Average", color='darkred', linewidth=2)

    plt.ylabel("Score")
    plt.xlabel("Episode")
    plt.title("Dueling DQN Training Progress (LunarLander-v3)")
