...
def train():
    ...
    print(f"Starting DQN training on {DEVICE} for {ENV_NAME}...")
    ...
    for ep in range(1, EPISODES + 1):
        ...
        if ep % 20 == 0:
            ...
            if avg_eval > best_score and avg_eval > 0:
                best_score = avg_eval
                torch.save(policy_net.state_dict(), "lunar_dqn_best.pth")
                print(f"--- New Best Model Saved: {best_score:.1f} ---")

    # Save final results
    torch.save(policy_net.state_dict(), "lunar_dqn_final.pth")
    np.save("lunar_dqn_scores.npy", np.array(training_scores))
